...
window.addEventListener("DOMContentLoaded",()=>{
    ...
    const $middle_menu_ = document.querySelector("header>.headcont>nav>.navCon>.gnb");
    ...
    window.addEventListener('scroll',()=>{
        ...
        console.log("예상 사이드바 이동거리 : ",$screenSize * 0.2,"px");
        let $nowScrollY = document.querySelector('html').scrollTop;
        console.log("현재 스크롤 Y : ",$nowScrollY);
        sideBar_toggle($nowScrollY,$screenSize);
        
    });


    //FIXME 사이드바 스크롤 액션 고치기
    function sideBar_toggle(scrollY,wSize) {
        ...
        if(scrollY > $middle_menu_.offsetTop){
            ...
            $sideBar.style =`display: block; transform: translateX(${wSize * -0.1}px)`;

        }else if(scrollY === 0) {
            // console.log("현재 스크롤Y가 section article_1보다 작습니다.");
            // console.log("$sideBar.style.display 를 이동시킨 후 none으로 변경합니다.");
            $sideBar.style =`transform: translateX((${wSize * 0.1}px)`;
            // setTimeout(()=>{$sideBar.style='display:none;'},1300);
        }
    }
